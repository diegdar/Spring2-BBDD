...
SELECT  nombre, apellido1, apellido2 FROM persona WHERE fecha_nacimiento LIKE "%1999%" ;
...
SELECT dep.nombre AS departamento, per.apellido1, per.apellido2, per.nombre FROM persona per RIGHT JOIN profesor prof ON prof.id_profesor = per.id JOIN departamento dep ON dep.id = prof.id_departamento ORDER BY dep.nombre, per.apellido1, per.apellido2, per.nombre;
-- 2.	Devuelve un listado con los profesores/as que no están asociados a un departamento. ->Resp: no hay resultados porque todos tienen un departamento
SELECT per.apellido1, per.apellido2, per.nombre FROM persona per LEFT JOIN profesor prof ON prof.id_profesor = per.id JOIN departamento dep ON dep.id = prof.id_departamento WHERE prof.id_profesor IS NULL;
-- 3.	Devuelve un listado con los departamentos que no tienen profesores asociados.
SELECT dep.id, dep.nombre AS departamento FROM persona per JOIN profesor prof ON prof.id_profesor = per.id RIGHT JOIN departamento dep ON dep.id = prof.id_departamento WHERE prof.id_departamento IS NULL;
-- 4. Devuelve un listado con los profesores/as que no imparten ninguna asignatura.
SELECT per.nombre, per.apellido1, per.apellido2, prof.id_profesor FROM persona per JOIN profesor prof ON prof.id_profesor = per.id LEFT JOIN asignatura asig ON asig.id_profesor = prof.id_profesor WHERE asig.id_profesor IS NULL;
...
-- 6.	🕰️🕰️APARCADO(NO ME SALE)->Devuelve un listado con todos los departamentos que no han impartido asignaturas en ningún curso escolar.
SELECT dep.nombre FROM departamento dep  LEFT JOIN profesor prof ON dep.id = prof.id_departamento JOIN asignatura asig ON asig.id_profesor = prof.id_profesor JOIN persona per ON per.id = prof.id_profesor JOIN alumno_se_matricula_asignatura am 	ON am.id_alumno = per.id JOIN curso_escolar ce ON ce.id = am.id_curso_escolar WHERE prof.id_profesor IS NULL AND ce.id IS NULL;
	/*Consultas resumen: */
...
SELECT ce.anyo_inicio AS anyo_incio_curso, COUNT(am.id_alumno) AS num_alumnos FROM alumno_se_matricula_asignatura am  JOIN asignatura asig ON asig.id = am.id_asignatura JOIN curso_escolar ce ON ce.id = am.id_curso_escolar GROUP BY anyo_incio_curso;
